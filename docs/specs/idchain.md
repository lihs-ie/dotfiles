...
## 固定スコープの委任実行（2026-09-08）

通常の人間承認モードを維持し、明示的に委任された成果についてだけ連続実行できるようにする。
契約は利用者成果、受入条件、対象外、対象IDと条件の対応、上流anchor、実行者、許可判断、有限予算、停止条件を固定する。
人間の直接承認とエージェントの委任判断は別のauthorityとして記録する。通常approveのnoteを委任台帳の代用にしない。

機械検査は契約・履歴の改変、対象外判断、予算超過、証拠欠落を拒否し、G2意味レビューとTC順序を維持する。
範囲内かどうかの意味判断は固定契約を渡した独立レビュアーが担い、新たな要求をレビューから追加しない。
準備・レビューの失敗も累積予算に含め、ID変更や別契約への付け替えで同じ作業を再開始しない。
完了後は判断追加を止め、次の成果を自動選択しない。未取得のrelease証拠を代理承認で埋めない。

受入検証は実CLIで委任登録→範囲内判断→check/reportを一周させ、契約改変・対象外・予算超過・未検証完了が
副作用なしで拒否されることを確認する。従来fixtureのhuman mode、Codex同期・配布も回帰検査する。
ローカルGit/コードを書き換えられる悪意ある操作者の認証・強制隔離は対象外であり、FNVハッシュを暗号的な認証と称さない。
