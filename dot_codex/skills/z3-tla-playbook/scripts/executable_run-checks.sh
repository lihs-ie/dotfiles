...
#
# モデル process の self-check exit 契約 (個々の HOLDS/REFUTED/ERROR 分類とは別):
#   0        全主張が宣言した期待結果と一致 (期待 REFUTED が実際 REFUTED でも 0)
#   1        期待結果との不一致 (broken variant はこれを期待)
#   2 以上   ERROR (依存不足・solver unknown・モデル不備・クラッシュ)。検査結果ではない
...
# TLC の stdout/stderr と exit code を、Java 非依存で三値分類する純粋関数。
classify_tlc_output() {
  local out_file="$1" exit_code="$2"
  if [ "$exit_code" -eq 0 ]; then
    printf 'OK'
  elif grep -qE 'Assumption .* is false' "$out_file"; then
    printf 'ERROR'
  elif grep -qE 'Invariant .* is violated|Temporal properties were violated|Deadlock reached|The behavior up to this point is' "$out_file"; then
    printf 'VIOLATE'
  else
    printf 'ERROR'
  fi
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --classify-tlc-output)
      classify_file="${2:?--classify-tlc-output needs FILE and EXIT}"
      classify_exit="${3:?--classify-tlc-output needs FILE and EXIT}"
      shift 3 ;;
...
if [ -n "$classify_file" ]; then
  [ -f "$classify_file" ] || { echo "no such TLC output: $classify_file" >&2; exit 2; }
  case "$classify_exit" in *[!0-9]*|'') echo "TLC exit must be a non-negative integer" >&2; exit 2 ;; esac
  classify_tlc_output "$classify_file" "$classify_exit"
  exit 0
fi
...
classify_tlc() {
  ...
  classify_tlc_output "$out_file" "$exit_code"
}
