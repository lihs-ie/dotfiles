...
委任実行では [idchain-approve](../idchain-approve/SKILL.md) の固定契約を読む。
追加する実装・回帰TC・準備作業を既存受入条件へ対応付ける。新しい要件をレビュー指摘だけで追加しない。
実装担当と独立レビュアーを分離し、修正は契約の累積予算内で最大2周。未解決なら範囲を広げず停止理由を報告する。
...
以下の対象範囲の違反を**全件ゼロ**にするまで、予算内で修正する:
...
- 対象の不合格は予算内で手順1〜5へ戻って直す。無関係な既存失敗の解消を新しい責務にしない。
  FAILをPASSへ読み替えず、対象成果の達成とglobal reportの状態を分けて報告する。
...
- レビュー観点: 品質 / セキュリティ / 設計と、固定契約・受入条件との対応
  (実装の意図説明や「なぜこう書いたか」は渡さない)

固定受入条件に必要なCRITICAL/HIGHの具体的反例だけを予算内で修正し、手順4〜6を再実行する。
severityだけを根拠に範囲外の機能を追加しない。
...
- 委任の終了条件を満たしたら、検証証拠を結合して完了を記録し停止する。リリース権限や外部証拠がなければ
  実装・検証完了とrelease blockedを区別する。G3未達を完了済みと称さず、追加改善・次RMを開始しない。
