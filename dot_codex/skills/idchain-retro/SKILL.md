...
G3は実リリースと固定指標の実測が揃い、かつその判断が委任されている場合だけ代理判断する。
不足証拠を生成・推定して埋めない。手元で取得できる実測を先に確認し、既存証拠を再質問しない。
本書の次RM起票・優先度変更は委任契約を拡張する権限ではない。固定成果を閉じたら終了し、追加改善は候補記録に留める。
