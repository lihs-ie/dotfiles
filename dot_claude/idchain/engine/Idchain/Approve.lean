...
private def renderRecord (record : ApprovalRecord) : String :=
  s!"  \{ target := ⟨Idchain.ArtifactKind.{record.target.kind.leanConstructor}, {record.target.number}⟩,
    approval := \{ approvedBy := {reprStr record.approval.approvedBy},
      date := {reprStr record.approval.date},
      note := {reprStr record.approval.note},
      contentHash := 0x{renderHash record.approval.contentHash} } }"
