...
- **フレーキーテストは `ci/quarantine.yml` に隔離** (`failure_class=flaky` が 2 回以上で義務)。
...
- ① 証跡が揃う (`commands.txt` / `wiring-map.json` / `completion-report.md`)
  ...
  ③ `iterations.json` の `failure_class` が全て valid (verify-failure-class.sh が exit 0)。

### Invariants (不変条件)
- context 窓 20% 以下 / wall clock (light=30min / heavy=90min) 超過 / no-new-evidence (20min 連続で新証拠なし) → Step 10 に強制ジャンプし `time-budget-exceeded.md` を残す。
- `failure_class=flaky` が 2 回以上で `ci/quarantine.yml` への隔離エントリ追加を implementer に義務付ける。
